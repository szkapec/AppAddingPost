...
const Form = ({currentId, setCurrentId, active, setActive}) => {
    ...
    return (
        <section className="form-container" className={active?"form-container":"form-container-hide"}>
            ...
            <form autoComplete="off" noValidate onSubmit={handleSubmit} className="form">
                ...
                <div>
                    <textarea className="message" name="message" placeholder="Wiadomość" variant="outlined" label="Message" value={postData.message} onChange={(e) => setPostData({ ...postData, message: e.target.value})}/>
                </div>
                ...
                <div>
                    ...
                    <button className="form-button" type="submit" onClick={()=> setActive(!active)}>{currentId?'Edytuj ':'Dodaj'}</button>
